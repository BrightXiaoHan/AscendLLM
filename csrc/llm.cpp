...
bool GemmRunner::PrepareInputs()
{
    size_t fileSize;
    // Read matrix A
    char *fileData = ReadFile("matrix_a.bin", fileSize, hostMatrixA_, sizeA_);
    if (fileData == nullptr) {
    ...
    // Read matrix B
    fileData = ReadFile("matrix_b.bin", fileSize, hostMatrixB_, sizeB_);
    if (fileData == nullptr) {
    ...
    // Read matrix C
    fileData = ReadFile("matrix_c.bin", fileSize, hostMatrixC_, sizeC_);
    if (fileData == nullptr) {
